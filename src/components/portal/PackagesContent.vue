...
<template>
  <div class="space-y-8">
    <!-- Header -->
    <div>
      <h2 class="text-foreground text-2xl font-semibold">服务订购</h2>
      <p class="text-muted-foreground">一站式 AI 能力套餐，整合大语言模型、向量服务与视觉服务，按需选择，即刻启程</p>
    </div>

    <!-- Package Cards -->
    <div class="xxl:grid-cols-4 grid items-stretch gap-6 md:grid-cols-1 lg:grid-cols-3 xl:grid-cols-3">
    ...
    <Dialog v-model:open="dialogOpen">
      <DialogContent>
        ...
        <div v-if="selectedPackage" class="space-y-4">
          <div class="bg-muted flex items-center gap-3 rounded-lg p-4">
          ...
          <div class="bg-muted/50 flex items-center gap-2 rounded-lg p-3">
            <InformationCircleIcon class="text-muted-foreground h-4 w-4 shrink-0" />
            <p class="text-muted-foreground text-xs">订购后将立即开通全部套餐内服务，费用将计入当月账单。</p>
          </div>
        </div>
        <DialogFooter>
