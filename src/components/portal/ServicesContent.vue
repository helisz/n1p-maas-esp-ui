...
import { KeyIcon, DocumentDuplicateIcon, ArrowPathIcon, TrashIcon, ExclamationTriangleIcon, CheckCircleIcon, SignalIcon, ArrowTrendingUpIcon, ClockIcon, ServerIcon } from '@heroicons/vue/24/outline';
...
const stats7d = [
  ...
  { date: '03-11', calls: 2200 },
  { date: '03-12', calls: 4800 },
  { date: '03-13', calls: 4100 },
  { date: '03-14', calls: 2500 },
  { date: '03-15', calls: 1200 },
  { date: '03-16', calls: 3800 },
];

const stats30d = [
  { date: '02-15', calls: 3200 },
  { date: '02-16', calls: 4100 },
  { date: '02-17', calls: 3800 },
  { date: '02-18', calls: 4500 },
  { date: '02-19', calls: 5200 },
  { date: '02-20', calls: 4900 },
  { date: '02-21', calls: 5600 },
  { date: '02-22', calls: 6100 },
  { date: '02-23', calls: 5800 },
  { date: '02-24', calls: 5300 },
  { date: '02-25', calls: 4700 },
  { date: '02-26', calls: 5100 },
  { date: '02-27', calls: 6200 },
  { date: '02-28', calls: 5900 },
  { date: '02-29', calls: 6500 },
  { date: '03-01', calls: 6800 },
  { date: '03-02', calls: 7200 },
  { date: '03-03', calls: 6900 },
  { date: '03-04', calls: 7400 },
  { date: '03-05', calls: 7100 },
  { date: '03-06', calls: 7800 },
  { date: '03-07', calls: 7500 },
  { date: '03-08', calls: 8200 },
  { date: '03-09', calls: 7900 },
  { date: '03-10', calls: 4500 },
...
const stats90d = [
  ...
  { date: 'W8', calls: 42100 },
  { date: 'W9', calls: 40800 },
  { date: 'W10', calls: 44500 },
  { date: 'W11', calls: 43200 },
  { date: 'W12', calls: 46800 },
];
...
function handleDeleteKey(key: (typeof apiKeys)[0]) {
  selectedKey.value = key;
...
function confirmDeleteKey() {
  if (selectedKey.value) {
    const index = apiKeys.findIndex((k) => k.id === selectedKey.value!.id);
    if (index !== -1) {
...
<template>
  <div class="space-y-6">
    <div>
      <h2 class="text-foreground text-2xl font-semibold">服务管理</h2>
    ...
    <Tabs default-value="packages" class="space-y-6">
          ...
          <Card v-for="pkg in myPackages" :key="pkg.id">
            <CardHeader class="pb-2">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                  <div class="bg-muted flex h-10 w-10 items-center justify-center rounded-lg"><ServerIcon class="h-5 w-5" /></div>
                  <div>
            ...
            <CardContent class="space-y-4">
              <div class="space-y-2">
                ...
                <p class="text-muted-foreground text-xs">已使用 {{ Math.round((pkg.usedCalls / pkg.totalCalls) * 100) }}%，剩余 {{ (pkg.totalCalls - pkg.usedCalls).toLocaleString() }} 次</p>
              </div>
              <div class="bg-muted/50 flex items-center gap-2 rounded-lg p-3">
                <span class="text-muted-foreground text-sm">API 端点：</span>
                <span class="flex-1 text-sm">{{ pkg.apiEndpoint }}</span>
      ...
      <TabsContent value="keys" class="space-y-4">
        <Card>
          ...
          <CardContent>
            <Table>
              <TableHeader>
                <TableRow> <TableHead>名称</TableHead><TableHead>AccessKey</TableHead><TableHead>创建时间</TableHead><TableHead>状态</TableHead><TableHead class="text-right">操作</TableHead> </TableRow>
              </TableHeader>
              <TableBody>
                <TableRow v-for="key in apiKeys" :key="key.id">
                  ...
                  <TableCell
                    ><Badge variant="outline" class="gap-1"><CheckCircleIcon class="h-3 w-3 text-green-500" />正常</Badge></TableCell
                  >
                  <TableCell class="text-right">
                    <Button variant="ghost" size="sm" @click="handleResetKey(key)"><ArrowPathIcon class="mr-1 h-3 w-3" />重置</Button>
                    <Button variant="ghost" size="sm" class="text-red-600 hover:text-red-700" @click="handleDeleteKey(key)"><TrashIcon class="mr-1 h-3 w-3" />删除</Button>
                  </TableCell>
                </TableRow>
              </TableBody>
            </Table>
      ...
      <TabsContent value="monitor" class="space-y-4">
        <div class="grid gap-4 md:grid-cols-3">
          <Card>
            <CardHeader class="pb-2"><CardTitle class="text-muted-foreground text-sm font-medium">今日调用</CardTitle></CardHeader>
            <CardContent
              ><div class="text-2xl font-bold">6,800</div>
              <div class="text-muted-foreground flex items-center text-xs"><ArrowTrendingUpIcon class="mr-1 h-3 w-3 text-green-500" />较昨日 +8%</div></CardContent
            >
          </Card>
          <Card>
            <CardHeader class="pb-2"><CardTitle class="text-muted-foreground text-sm font-medium">本周调用</CardTitle></CardHeader>
            <CardContent><div class="text-2xl font-bold">40,100</div></CardContent>
          </Card>
          <Card>
            <CardHeader class="pb-2"><CardTitle class="text-muted-foreground text-sm font-medium">错误率</CardTitle></CardHeader>
        ...
        <Card>
          ...
          <CardContent>
            <Table>
              <TableHeader
                ><TableRow><TableHead>时间</TableHead><TableHead>API</TableHead><TableHead>错误</TableHead><TableHead>状态码</TableHead></TableRow></TableHeader
              >
              <TableBody>
                <TableRow v-for="log in errorLogs" :key="log.time">
                  <TableCell class="text-muted-foreground">{{ log.time }}</TableCell>
                  <TableCell class="text-sm">{{ log.api }}</TableCell>
                  <TableCell
                    ><div class="flex items-center gap-2"><ExclamationTriangleIcon class="h-4 w-4 text-yellow-500" />{{ log.error }}</div></TableCell
                  >
                  <TableCell
                    ><Badge variant="outline" class="gap-1 text-red-500">{{ log.code }}</Badge></TableCell
                  >
                </TableRow>
              </TableBody>
            </Table>
    ...
    <Dialog v-model:open="resetDialogOpen">
      <DialogContent>
        ...
        <div v-if="selectedKey" class="border-destructive/50 bg-destructive/5 rounded-lg border p-4">
          <p class="text-sm">
            确定要重置 <span class="font-medium">{{ selectedKey.name }}</span> 的 SecretKey 吗？
          </p>
        </div>
        <DialogFooter>
    ...
    <Dialog v-model:open="deleteDialogOpen">
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>此操作不可逆，删除后该密钥将立即失效</DialogDescription>
        </DialogHeader>
        <div v-if="selectedKey" class="border-destructive/50 bg-destructive/5 rounded-lg border p-4">
          <p class="text-sm">
            确定要删除 <span class="font-medium">{{ selectedKey.name }}</span> 吗？
          </p>
        </div>
        <DialogFooter>
          <Button variant="outline" @click="deleteDialogOpen = false">取消</Button>
          <Button variant="destructive" @click="confirmDeleteKey">确认删除</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  </div>
